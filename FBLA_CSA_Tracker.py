# Lines 1-9 import the libraries needed to program the application.
# Python Libraries used: tkinter, cx_Oracle, itertools, python-docx, os
from tkinter import *
import cx_Oracle
import itertools
...
# In order to view/edit/add data to the database,
# the application must connect to the database using the correct username and password
# The variable 'connstr' contains the information needed to connect to the database
connstr = 'fbla_csa/fbla_csa@fbladb1'

# The empty lists 'labels', 'buttons', 'tboxes', and 'dropdowns' store each object in the application.
labels = []
buttons = []
tboxes = []
dropdowns = []

# The 'destroy_all()' function refreshes the page by destroying all objects from the previously chosen page
# so that the page currently being viewed can load all of its objects
def destroy_all():
   for label in labels:
      label.destroy()
...
# Queries community service awards in the query form
def query_csa():
   destroy_all()
...
# Queries community service hours in the query form
def query_csa_hour():
   destroy_all()
...
# Queries award categories in the query form
def query_award_det():
   destroy_all()
...
# Adds New Student in Add Entry Form
def add_student():
   destroy_all()
...
# Adds community service details
def add_cs():
   destroy_all()
...
# Modifies (updates/deletes) student data 
def modify_student():
   destroy_all()
...
# Modify community service hours data
def modify_cshrs():
   destroy_all()
...
# Prints community service reports for a date range
def print_report():
   destroy_all()
...
# Backup FBLA CSA data
def backup():
   destroy_all()
   
   def backup_files():
      os.system('expdp fbla_csa/fbla_csa directory=fbla_backup dumpfile=fbla_csa.dmp schemas=fbla_csa reuse_dumpfiles=YES')
...
# Creates the application window for FBLA_CSA_Program
root = Tk()
...
# Adds commands to the edit menu
addmenu.add_command(label="Student", command=add_student)
addmenu.add_command(label="Chapter", command=add_chapter)
addmenu.add_command(label="Award Detail", command=add_award)
addmenu.add_command(label="Community Service", command=add_cs)
addmenu.add_command(label="Coummunity Service Hours", command=add_cshrs)
menubar.add_cascade(label="Add Data", menu=addmenu)
...
# Adds commands to the search menu
searchmenu.add_command(label="Chapter", command=query_chapter)
searchmenu.add_command(label="Student", command=query_student)
searchmenu.add_command(label="Program Category", command=query_progcat)
searchmenu.add_command(label="Community Service", command=query_csa)
searchmenu.add_command(label="Coummunity Service Hours", command=query_csa_hour)
searchmenu.add_command(label="Award Detail", command=query_award_det)
menubar.add_cascade(label="Search Data", menu=searchmenu)

# Creates the print menu
printmenu = Menu(menubar, tearoff=0)
# Adds command to print menu
...
# Adds commands to the option menu
optionmenu.add_command(label='Backup FBLA Data',command=backup)
optionmenu.add_command(label="Quit", command=root.destroy)
